...
<template>
  <section class="flex flex-wrap justify-center bg-section dark:bg-darkmode" id="projects">
      ...
      <div class="grid grid-cols-12 gap-7">
        ...
        >
          <div class="relative overflow-hidden md:col-span-7">
            <router-link :to="`/project/${projectsPost[0].slug}`" :aria-label="projectsPost[0].title || 'Artículo destacado'" class="block h-full">
              <img
                :src="getCoverImage(projectsPost[0])"
                :alt="projectsPost[0].title || 'Artículo destacado'"
                class="h-100 w-100 object-cover transition duration-500 group-hover:scale-105"
            ...
            <div class="absolute left-4 top-4 rounded-full bg-primary px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white shadow-lg">
              {{ projectsPost[0].category || 'Laravel' }}
            </div>
          </div>
        ...
        <div
          v-for="(project, i) in projectsPost.slice(1)"
          :key="project.slug || i"
        ...
        >
          <article class="group relative flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-service transition hover:-translate-y-1 dark:bg-darkmode">
            <div class="relative overflow-hidden">
              <router-link :to="`/project/${project.slug}`" :aria-label="project.title || 'Artículo del blog'" class="block">
                <img
                  :src="getCoverImage(project)"
                  :alt="project.title || 'Artículo del blog'"
                  class="w-full h-100 object-cover transition duration-500 group-hover:scale-105"
                />
              </router-link>

              <div class="absolute left-4 top-4 rounded-full bg-primary px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white shadow-lg">
                {{ project.category || 'Oncología' }}
              </div>
            </div>
